...
const restore = async() => {
  ...
  file.onchange = async (e) => {
    ...
    reader.onload = async () => {
      ...
      const [, setSettings] = useFetch(() => r.post("/admin/setting/save", data.setting));
          ...
          if (resp.code == 200) {
            for (let index in data.users) {
              ...
              for (let index1 in resp.data.content) {
                if (data.users[index].username == resp.data.content[index1].username) {
                  method = 'save'
                }
              }
              if (method == 'save') {
              const [, setUsers] = useFetch(() => r.post("/admin/user/update", data.users[index]));
              ...
              else if (data.users[index].username == 'admin' || data.users[index].username == 'guest') {}
              else {
                const [, setUsers] = useFetch(() => r.post("/admin/user/create", data.users[index]));
                const setUsers1 = async () => {
                  const resp: PageResp<User> = await setUsers();
                  handleResp(resp, () => {
                    if (resp.code == 200)
        ...
        handleResp(resp, async () => {
          if (resp.code == 200) {
            for (let index in data.metas) {
              ...
              if (method == 'save') {
              const [, setMetas] = useFetch(() => r.post("/admin/meta/update", data.metas[index]));
              ...
              await setMetas1();
                ...
                const [, setMetas] = useFetch(() => r.post("/admin/meta/create", data.metas[index]));
                const setMetas1 = async () => {
                  const resp: PageResp<Meta> = await setMetas();
                  handleResp(resp, () => {
                    if (resp.code == 200)
        ...
        handleResp(resp, async () => {
          if (resp.code == 200) {
            for (let index in data.storages) {
              var method = 'add'
              for (let index1 in resp.data.content) {
                if (data.storages[index].mount_path == resp.data.content[index1].mount_path) {
                  method = 'save'
                }
              }
              if (method == 'save') {
              const [, setStorages] = useFetch(() => r.post("/admin/storage/update", data.storages[index]));
              ...
              await setStorages1();
                ...
                const [, setStorages] = useFetch(() => r.post("/admin/storage/create", data.storages[index]));
                const setStorages1 = async () => {
                  const resp: PageResp<Storage> = await setStorages();
                  handleResp(resp, () => {
                    if (resp.code == 200)
